...
class SliderController extends Controller
{
    public function index(Request $request, ListSlidersAction $action): JsonResponse
    {
        return ApiResponse::paginated(
            $action->execute(app('current_tenant')->id, $request->integer('per_page', 15), SliderFilterData::fromRequest($request)),
    ...
    public function store(StoreSliderRequest $request, CreateSliderAction $action): JsonResponse
    {
        return ApiResponse::created(
            new SliderResource($action->execute($request->user()->currentTenantId(), $request->toDTO())),
