...
class OfferController extends Controller
{
    public function index(Request $request, ListOffersAction $action): JsonResponse
    {
        return ApiResponse::paginated(
            $action->execute(app('current_tenant')->id, $request->integer('per_page', 15), OfferFilterData::fromRequest($request)),
            OfferResource::class,
    ...
    public function store(StoreOfferRequest $request, CreateOfferAction $action): JsonResponse
    {
        return ApiResponse::created(
            new OfferResource($action->execute($request->user()->currentTenantId(), $request->toDTO())),
